...
//<ASUS-Darren1_Chang20120627+>
#ifdef CONFIG_ASUS_ENGINEER_MODE
struct i2c_client   *ex_client = NULL;
#endif
//<ASUS-Darren1_Chang20120627->
...
#ifdef CONFIG_ASUS_ENGINEER_MODE
	case ASUS_IOCTL_GET_X_AXIS:
		if(ex_client == NULL)
			return -EFAULT;
		//X_AXIS			
		ret = i2c_smbus_read_word_data(ex_client, X_AXIS);
		ret = ret >> 4;
#ifdef DEBUG
		printk("X_AXIS : %d / 0x%04X\n", ret, ret);
#endif
		return	ret;
		break;
	case ASUS_IOCTL_GET_Y_AXIS:
		if(ex_client == NULL)
			return -EFAULT;
		//Y_AXIS
		ret = i2c_smbus_read_word_data(ex_client, Y_AXIS);
		ret = ret >> 4;
#ifdef DEBUG
		printk("Y_AXIS : %d / 0x%04X\n", ret, ret);
#endif
		return	ret;
		break;
	case ASUS_IOCTL_GET_Z_AXIS:
		if(ex_client == NULL)
			return -EFAULT;
		//Z_AXIS
		ret = i2c_smbus_read_word_data(ex_client, Z_AXIS);
		ret = ret >> 4;
#ifdef DEBUG
		printk("Z_AXIS : %d / 0x%04X\n", ret, ret);
#endif
		return	ret;
		break;
#endif
