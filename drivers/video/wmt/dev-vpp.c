...
//	DBGMSG("vout_ioctl\n");
	...
	//			DBGMSG("VPPIO_VOGET_INFO\n");
				...
				if( (g_vpp.dual_display == 1) && (g_vpp.virtual_display == 0) && (fb_num != vo_info->num) ){
					retval = -ENOTTY;
					break;
				}
...
void vpp_var_to_fb(struct fb_var_screeninfo *var, struct fb_info *info,vdo_framebuf_t *fb)
{
	...
	if( var ){
		if( !(fb->col_fmt = WMT_FB_COLFMT(var->nonstd)) ){
		...
		addr = info->fix.smem_start + (var->yoffset * var->xres_virtual * ((y_bpp + c_bpp) >> 3));
...
void vpp_set_lvds_blank(int blank)
{
   ...
   // printk("%s %d\n",__FUNCTION_,blank);
	...
	vout_set_blank((0x1 << VPP_VOUT_NUM_LVDS),(blank)? VOUT_BLANK_POWERDOWN:VOUT_BLANK_UNBLANK);
